...
function preload(p5){
    ...
    loadMarker(p5, 'cross-02.png')
    loadMarker(p5, 'star-02.png')

    sshader = p5.loadShader('filters/filter3/shader.vert', 'filters/filter3/shader.frag')

    colors = [p5.color('#ffffff'), p5.color('#ffdbdb'), p5.color('#ffa5a5'), p5.color('#fc6060'), p5.color('#fc2121'), p5.color('#ad1919'), p5.color('#4F4F4F'), p5.color('#323232')]
    colorsTwo = [p5.color('#fc2121'), p5.color('#ffffff')]
}
...
function draw(p5, portrait){
    ...
    const symbols1 = filterSymbols([0,1])
            
    if(symbols1.length > 0){
        runPixelsEqual(p5, Number(quality.value), symbols1, colors, portrait)
    }
    
    const symbols2 = filterSymbols([2])

    if(symbols2.length > 0){
        runPixelsScaled(p5, 150, symbols2, colorsTwo, portrait)
    }
}
