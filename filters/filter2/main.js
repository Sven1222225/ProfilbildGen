...
function preload(p5){
    ...
    loadMarker(p5, 'circle-41.png')
    loadMarker(p5, 'circle-45.png')
    ...
    sshader = p5.loadShader('filters/filter2/shader.vert', 'filters/filter2/shader.frag')

    colors = [p5.color('#ffffff'), p5.color('#dddbff'), p5.color('#a5a9ff'), p5.color('#6067fc'), p5.color('#212afc'), p5.color('#191fad'), p5.color('#4F4F4F'), p5.color('#323232')]
    colorsTwo = [p5.color('#212afc'), p5.color('#ffffff')]
}
...
function draw(p5, portrait){
    ...
    const symbols2 = filterSymbols([4,5,6,7,8,9])
        ...
        for (let x = 0; x < portrait.width; x += stepSize){
            ...
            if(performanceMode){
                ...
                p5.push()
                    ...
                    p5.translate((-portrait.width / 2) + x + p5.random(-12, 2), (-portrait.height / 2) + y + p5.random(-2, 12))
                    p5.rotate(p5.random(0, 270))
                    p5.image(pixelSet[getRandomInt(pixelSet.length)], 0, 0, radius, radius) //slow method
                    
                p5.pop()
                p5.tint(colorSet[activeColor])
            }
        }
    }
}
    ...
    for (let y = 0; y < portrait.height; y += stepSize){
        for (let x = 0; x < portrait.width; x += stepSize){
            ...
            if(performanceMode){
                ...
                p5.image(pixelSet[getRandomInt(pixelSet.length)], (-portrait.width / 2) + x + p5.random(-12, 12), (-portrait.height / 2) + y + p5.random(-12, 12), radius, radius)
                p5.rotate(p5.random(-36, 36))
                p5.tint(colorSet[activeColor])
            }
